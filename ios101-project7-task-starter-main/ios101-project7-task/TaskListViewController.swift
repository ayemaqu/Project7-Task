...
class TaskListViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        // Create and configure the progress bar programmatically
        progressBar = UIProgressView(progressViewStyle: .default)
        progressBar.frame = CGRect(x: 50, y: 750, width: 288, height: 20)
        view.addSubview(progressBar)

        // Call updateProgressBar after progressBar is initialized
        updateProgressBar()
            ...
            if let composeNavController = segue.destination as? UINavigationController,
               ...
               let composeViewController = composeNavController.topViewController as? TaskComposeViewController {
                ...
                composeViewController.onComposeTask = { [weak self] task in
                    ...
                    // Update the progress bar after a task is composed
...
extension TaskListViewController: UITableViewDelegate {
    ...
    // Addition progress bar
    func updateProgressBar() {
        let progress = Task.overallProgress()
        progressBar.setProgress(progress, animated: true)
    }

    // Call this method whenever the completion status of tasks changes
    func tasksDidChange() {
        updateProgressBar()
    }
